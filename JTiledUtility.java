...
public class JTiledUtility {
    ...
    public <T extends JGameEngine.Object> void setGameSpace(String filename, ArrayList<Class> clas) {
	Gson gson = new Gson();
	...
	try {
		...
		for(int i = 0; i < layers.size(); i++) {
			...
			if(clas == null)
			    solveObjects(layer, JTiledObject.class);
			else
			    solveObjects(layer, clas.get(objectgroupcount));
			objectgroupcount++;
		    }
		}
    ...
    private <T extends JGameEngine.Object> void solveObjects(JsonObject layer, Class<T> clas) {
	...
	try {
	    ...
	    if(ctor == null) System.out.println("JTiledUtility::setGameSpace() Class needs to have a constructor that only has 1 argument that accepts a JGameEngine");
	    T standardObj = (T) ctor.newInstance(e);
	    int maskCount = 0;
		...
		if(jobj.get("name").getAsString().equals("")) {
		    ...
		    JTiledObject newObj = new JTiledObject(e);
		    addObjectMask(newObj, jobj);
		    newObj.name = jobj.get("name").getAsString();
		    e.addObject(newObj);
		}
	    }
	    if(maskCount != 0) e.addObject(standardObj);
	} catch(Exception e) { System.out.println("JTiledUtility::setGameSpace() invalid object class, skipping " + e.toString()); e.printStackTrace(); }
    }
...
class JTiledDraw extends JGameEngine.Object {
	...
	for(int i = 0; i < draw_layers.size(); i++) {
		...
		for(int k = 0; k < e.cameraWidth()/tile_width + 5; k++) {		    
			...
			(int) (((k * tile_width) - (e.cameraX() % tile_width)) * 1/e.cameraDistance()),// + (int) ((e.cameraX()/tile_width)*tile_width), 
			(int) (((j * tile_height) - (e.cameraY() % tile_height))* 1/e.cameraDistance()),// + (int) ((e.cameraY()/tile_height)*tile_height),
			(int) (tile_width * 1/e.cameraDistance()), 
			(int) (tile_height * 1/e.cameraDistance()), null);	
		    } 
		    k1++;
		} 
		j1++; k1 = 0;
